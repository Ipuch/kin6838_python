...
z_axis = z_axis[:, np.newaxis]
origin = origin[:, np.newaxis]

tuple_axis = (x_axis, y_axis, z_axis)
R01 = np.concatenate(tuple_axis, axis=1)

T01 = np.concatenate((R01, origin), axis=1)
T01 = np.concatenate(
    (
    T01, np.array([[0, 0, 0, 1]])
    ), 
    axis=0)
...
right_hip_in_pelvis = T10 @ right_hip_joint_center
# left_hip_in_pelvis = ...
